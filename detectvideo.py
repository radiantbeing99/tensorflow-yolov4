...
for p_device in physical_devices:
    tf.config.experimental.set_memory_growth(p_device, True)
...
def main(_argv):
    ...
    logging.info("Video from: ", video_path)
    ...
    if FLAGS.framework == 'tflite':
        ...
        # # tf saved_model style
        saved_model = tf.saved_model.load(FLAGS.weights, tags=[tag_constants.SERVING])
    ...
    while True:
        ...
        if return_value:
            ...
            logging.info(vid.get(cv2.CAP_PROP_FRAME_COUNT))
        ...
        if FLAGS.framework == 'tflite':
            ...
            # tf saved_model style
            pred_bbox = saved_model(image_data)
            boxes = pred_bbox[:, :, 0:4]
